...
var RunnableLambda = class RunnableLambda extends Runnable {
	...
	async _invoke(input, config, runManager) {
		return new Promise((resolve, reject) => {
			...
			require_index$1.AsyncLocalStorageProviderSingleton.runWithConfig(require_config.pickRunnableConfigKeys(childConfig), async () => {
				try {
...
var RunnableWithFallbacks = class extends Runnable {
	...
	*runnables() {
	...
	async invoke(input, options) {
		...
		const res = await require_index$1.AsyncLocalStorageProviderSingleton.runWithConfig(childConfig, async () => {
			let firstError;
